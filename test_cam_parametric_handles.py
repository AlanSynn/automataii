...
def test_cam_handles():
    ...
    print("   - Follower: (350, {:.1f})".format(520 + rod_handle_y))
    print("   - Shape handle: ({:.1f}, 520)".format(350 + shape_handle_x))
    
    print("\n2. DRAGGING ROD HANDLE:")
    print("   - Original: (0, {:.1f})".format(rod_handle_y))
    ...
    print("   - New position: (0, {:.1f})".format(new_rod_y))
    print("   - New scaled rod length: {:.1f}mm".format(new_rod_length))
    print("   - New unscaled rod length: {:.1f}mm".format(new_unscaled_rod))
    print("   - Effect: Follower moves closer/farther from CAM")
    
    print("\n3. DRAGGING SHAPE HANDLE:")
    print("   - Original: ({:.1f}, 0)".format(shape_handle_x))
    ...
    print("   - New position: ({:.1f}, 0)".format(new_shape_x))
    print("   - New max radius (unscaled): {:.1f}mm".format(new_max_radius))
    print("   - New base radius: {:.1f}mm".format(new_base))
    print("   - New eccentricity: {:.1f}mm".format(new_ecc))
