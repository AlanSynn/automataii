...
class PlanetaryGearBlueprintGenerator(BlueprintGenerator):
    ...
    def _generate_front_view(self, mechanism_data: Dict[str, Any]):
        ...
        # Ring gear (outer)
        self._add_gear_teeth(cx, cy, r_ring, int(r_ring / module), module, 
                            pressure_angle, internal=True)
        
        # Sun gear (center)
        self._add_gear_teeth(cx, cy, r_sun, int(r_sun / module), module, 
                            pressure_angle, internal=False)
        ...
        for i in range(num_planets):
            ...
            self._add_gear_teeth(px, py, r_planet, int(r_planet / module), 
                                module, pressure_angle, internal=False)
    ...
    def _add_gear_teeth(self, cx: float, cy: float, radius: float, 
                        num_teeth: int, module: float, pressure_angle: float,
                        internal: bool = False):
        """
        ...
        for i in range(num_teeth):
            ...
            if internal:
                ...
                points = []
                points.append((cx + inner_radius * math.cos(angles[0]),
                             cy + inner_radius * math.sin(angles[0])))
                points.append((cx + outer_radius * math.cos(angles[1]),
                             cy + outer_radius * math.sin(angles[1])))
                points.append((cx + outer_radius * math.cos(angles[2]),
                             cy + outer_radius * math.sin(angles[2])))
                points.append((cx + inner_radius * math.cos(angles[3]),
    ...
    def _add_part_list(self, mechanism_data: Dict[str, Any]):
        ...
        parts = [
            {"no": "1", "name": "Sun Gear", "qty": "1", "material": "Steel", 
             "spec": f"Module {params.get('module', 2)}"},
            {"no": "2", "name": "Planet Gear", "qty": str(num_planets), 
             "material": "Steel", "spec": f"Module {params.get('module', 2)}"},
            {"no": "3", "name": "Ring Gear", "qty": "1", "material": "Steel", 
             "spec": f"Internal, Module {params.get('module', 2)}"},
            {"no": "4", "name": "Planet Carrier", "qty": "1", 
             "material": "Aluminum", "spec": "6061-T6"},
            {"no": "5", "name": "Planet Pin", "qty": str(num_planets), 
             "material": "Steel", "spec": "Ø8mm x 30mm"},
            {"no": "6", "name": "Bearing", "qty": str(num_planets * 2 + 2), 
             "material": "-", "spec": "608ZZ"},
            {"no": "7", "name": "Input Shaft", "qty": "1", 
             "material": "Steel", "spec": "Ø10mm"},
            {"no": "8", "name": "Output Shaft", "qty": "1", 
             "material": "Steel", "spec": "Ø12mm"},
        ]
