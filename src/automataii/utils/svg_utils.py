...
def contour_to_svg_path(contour) -> str:
    ...
    if contour is None or len(contour) < 1:
        return ""
    path_data = "M " + " L ".join([f"{p[0]},{p[1]}" for p_arr in contour for p in p_arr]) # Adjusted for typical contour structure
    # If contour is already a list of points (e.g., [[x1,y1], [x2,y2]]), then:
    # path_data = "M " + " L ".join([f"{p[0]},{p[1]}" for p in contour])
    path_data += " Z"  # 경로 닫기
    return path_data

def save_svg(path_data: str, width: int, height: int, output_path: str,
             fill: str = "rgba(255,255,255,0.5)", stroke: str = "black", stroke_width: int = 1) -> bool:
    """SVG 파일로 저장합니다."""
