...
def setup_logging(console_log_level: int = logging.INFO):
    ...
    file_formatter = logging.Formatter('%(asctime)s - %(levelname)-8s - %(name)-25s - %(module)-20s - %(funcName)-25s - L%(lineno)-4d - %(message)s')
    file_handler.setFormatter(file_formatter)

    # Configure console handler
    console_handler = logging.StreamHandler()
    console_handler.setLevel(console_log_level) # Use passed level
    console_formatter = logging.Formatter('%(levelname)s: [%(name)s] %(message)s') # Slightly more info for console
