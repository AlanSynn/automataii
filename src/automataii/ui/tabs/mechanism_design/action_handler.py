...
class MechanismActionHandler(QObject):
    ...
    def _generate_mechanism_from_candidate(self, candidate_data):
        ...
        logger.info(f"ActionHandler: Generating mechanism from candidate data")
        logger.info(f"ActionHandler: Candidate data keys: {list(candidate_data.keys())}")
        logger.info(f"ActionHandler: Mechanism type: {candidate_data.get('type')}")
        
        part_name = self.state.selected_part_name
        logger.info(f"ActionHandler: Selected part: {part_name}")
        ...
        logger.info(f"ActionHandler: Converted parameters: {params}")
        ...
        layer_data = {
            ...
            "part_name": part_name,
            "params": params,
            "parameters": params,  # Add duplicate for compatibility
            ...
            "original_json_type": mechanism_type_value,  # Keep original type for visual factory
            "user_path_aligned_np": candidate_data.get("user_path_aligned_np"),  # For proper alignment
            "mech_path_aligned_np": candidate_data.get("mech_path_aligned_np"),  # For proper alignment
        }
        logger.info(f"ActionHandler: Created layer_data with id {mechanism_id}")
        ...
        logger.info(f"ActionHandler: Extracted key points: {list(key_points.keys()) if key_points else 'None'}")

        self._adjust_mechanism_to_target_joint(layer_data)

        logger.info(f"ActionHandler: Adding mechanism {mechanism_id} to state")
    ...
    def _adjust_mechanism_to_target_joint(self, layer_data: dict):
        ...
        logger.info(
            f"Adjusted mechanism '{layer_data['id']}' by offset {offset} to align with joint '{target_joint_id}'."
        )
    ...
    def handle_export_blueprint(self):
        ...
        try:
            from automataii.ui.exporters.blueprint_exporter import BlueprintExporter

            exporter = BlueprintExporter(
                self.state.parts_data, self.state.mechanism_layers, self.state.initial_skeleton_data
            )
            exporter.export(save_path)
            QMessageBox.information(
                self.main_window, "Success", f"Blueprint exported to {save_path}"
            )
