...
class MechanismControlPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.parts_list.setToolTip(
            "Click to toggle mechanism generation for a part. Black text: path available. Gray text: no path."
        )
    ...
    def _update_parts_list(self):
        ...
        sorted_parts = sorted(
            self.state.parts_data.keys(), key=lambda p: (p not in self.state.path_data, p)
        )
        ...
        for name in sorted_parts:
            ...
            has_path = (
                name in self.state.path_data
                and self.state.path_data[name]
                and not self.state.path_data[name].isEmpty()
            )
    ...
    def _update_button_states(self):
        ...
        has_enabled_paths = any(
            self.state.part_enabled_state.get(name)
            for name, path in self.state.path_data.items()
            if path and not path.isEmpty()
        )
        has_mechanisms = bool(self.state.mechanism_layers)

        self.recommendation_btn.setEnabled(
            has_enabled_paths and bool(self.state.selected_part_name)
        )
