...
class EditorSceneManager:
    ...
    def update_visuals_from_animation_data(self, joint_positions: dict[str, QPointF | tuple[float, float]]) -> None:
        """Update skeleton and parts from IK joint positions data."""
        
        # Normalize joint positions to consistent format
        joint_positions_tuples = {}
        joint_positions_qpointf = {}
        
        for joint_id, pos in joint_positions.items():
            if hasattr(pos, 'x'):  # QPointF
                joint_positions_tuples[joint_id] = (pos.x(), pos.y())
                joint_positions_qpointf[joint_id] = pos
            else:  # Already a tuple
                joint_positions_tuples[joint_id] = pos
                joint_positions_qpointf[joint_id] = QPointF(pos[0], pos[1])
        
        # Update skeleton visualization if it exists
        if self.skeleton_item:
            self.skeleton_item.set_animated_pose(joint_positions_tuples)
        else:
