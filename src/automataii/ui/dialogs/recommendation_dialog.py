...
class MechanismPreviewWidget(QGraphicsView):
    ...
    def _draw_cam_follower_from_sim(self, transform: QTransform, full_sim_data: dict, params: dict):
        ...
        self.scene.addPath(
            cam_path, QPen(QColor("#e74c3c"), 4), QBrush(QColor("#e74c3c").lighter(160))
        )
        ...
        self.scene.addPolygon(
            follower_poly, QPen(QColor("#2ecc71"), 3), QBrush(QColor("#2ecc71").lighter(160))
        )
...
class MechanismRecommendationDialog(QDialog):
    ...
    def _get_best_recommendations(self) -> list[dict[str, Any] | None]:
        ...
        print(
            f"Debug: Found {len(mechanisms_by_type)} mechanism types: {list(mechanisms_by_type.keys())}"
        )
        ...
        for i, mech in enumerate(top_3):
            print(
                f"Debug: Recommendation {i + 1}: {mech['type']} - {mech['name']} (score: {mech['overall_score']:.2f})"
            )
    ...
    def _get_mechanism_points_orig(self, mechanism_data: dict[str, Any]) -> np.ndarray | None:
        ...
        if mech_type == "4-bar Coupler" and key_points:
            ...
            if base_radius is not None and eccentricity is not None:
                ...
                cam_points = (
                    cam_center_orig + base_radius * np.array([np.cos(thetas), np.sin(thetas)]).T
                )
