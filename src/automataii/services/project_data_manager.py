...
from automataii.models.pydantic import (
...
from automataii.models.runtime import PartInfo  # Runtime PartInfo class
...
class ProjectDataManager(QObject):
    ...
    def raw_skeleton_data(self) -> list[dict[str, Any]] | None:
        ...
        # If no validated project data, return preserved skeleton data
    ...
    def load_project_from_file(self, filepath: str, preserve_skeleton: bool = False) -> bool:
        logging.info(f"ProjectDataManager: Attempting to load project from: {filepath}")
        self.clear_project_data(preserve_skeleton=preserve_skeleton)
        ...
        self.clear_project_data(preserve_skeleton=preserve_skeleton)  # Ensure clean state on error
        ...
        if part_name in self._parts:
            ...
            if self._validated_project_data and self._validated_project_data.character:
                if part_name in self._validated_project_data.character.parts:
                    ...
                    from automataii.models.pydantic import MotionPathDataModel, QPointFModel
