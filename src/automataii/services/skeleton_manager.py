...
class SkeletonManager(QObject):
    ...
    def is_skeleton_compatible_with_parts(self, parts_info: dict[str, Any]) -> bool:
        """
        Checks if current skeleton is compatible with given parts data.
        Returns True if all part anchor joints exist in skeleton.
        """
        if not self._standardized_skeleton_model:
            return False

        skeleton_joints = set(self._standardized_skeleton_model.joints.keys())

        for part_name, part_info in parts_info.items():
            if hasattr(part_info, "anchor_joint_id") and part_info.anchor_joint_id:
                if part_info.anchor_joint_id not in skeleton_joints:
                    logging.warning(
                        f"Part '{part_name}' references non-existent joint '{part_info.anchor_joint_id}'"
                    )
                    return False
    ...
    def _process_animated_drawings_format(
        ...
        for i, joint_info_raw in enumerate(raw_joints_list):
            ...
            # logging.info(f"AD Processing: Joint '{joint_name}' (ID: {joint_id}) has parent: '{parent_name}'")
        ...
        logging.info(f"AD Processing: Resolving parent relationships for {len(std_skeleton.joints)} joints")
        ...
        for joint_id, joint_model in std_skeleton.joints.items():
            ...
            if parent_name and parent_name in temp_joint_name_to_id:
                ...
                logging.info(f"AD Processing: Set parent_id for {joint_model.name}: {parent_id}")
        ...
        try:
            ...
            for root_id in self._standardized_skeleton_model.root_joint_ids:
                if root_id in self._standardized_skeleton_model.joints:
                    root_positions[root_id] = self._standardized_skeleton_model.joints[
                        root_id
                    ].position
