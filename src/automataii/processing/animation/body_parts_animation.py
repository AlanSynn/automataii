...
from .joint_connection_system import JointConnectionRenderer, QtJointConnectionHelper
...
def animate_body_part(
    ...
    joint_renderer=None, part_info=None, skeleton_data=None, connected_parts=None
):
    """
    ...
    for i in range(keyframe_count - 1):
        ...
        for t in range(1, num_frames + 1):
            ...
            try:
                ...
                # Apply joint connections if enabled
                if joint_renderer and joint_renderer.enabled and connected_parts:
                    # Prepare parts data for renderer
                    parts_data = {
                        part_info['name']: {
                            'image': deformed_frame,
                            'position': (part_info.get('x', 0), part_info.get('y', 0)),
                            'rotation': part_info.get('rotation', 0),
                            'scale': 1.0
                        }
                    }
                    
                    # Add connected parts data
                    for conn_name, conn_data in connected_parts.items():
                        if conn_name != part_info['name']:
                            parts_data[conn_name] = conn_data
                    
                    # Render with joint connections
                    rendered_parts = joint_renderer.render_connected_parts(parts_data)
                    deformed_frame = rendered_parts.get(part_info['name'], deformed_frame)
