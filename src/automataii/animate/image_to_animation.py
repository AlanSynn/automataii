...
def image_to_animation(
    ...
    logger.info(f"Starting image_to_animation for {img_fn}. Final output intended for {final_output_dir}")
    ...
    if not annotation_results:
        logger.error(f"Failed to generate annotations for {img_fn}. Aborting animation pipeline.")
        return

    temp_char_anno_dir = annotation_results['output_dir']
    logger.info(f"Annotations generated successfully in temporary directory: {temp_char_anno_dir}")
    ...
    try:
        ...
        logger.info(f"Animation processing complete for annotations in {temp_char_anno_dir}")
    ...
    logger.info(f"Copying animation results from {temp_char_anno_dir} to {final_output_dir}")
    ...
    if copied_something:
        ...
        logger.warning(f"No standard animation output (e.g., video.gif, frames/) found in {temp_char_anno_dir} to copy.")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        logger_main.error("Usage: python image_to_animation.py <image_path> <final_output_directory> [motion_cfg] [retarget_cfg]")
        sys.exit(1)
    ...
    logger_main.info(f"Running image_to_animation for image: {img_fn_arg}, output to: {final_output_dir_arg}")
    image_to_animation(img_fn_arg, final_output_dir_arg, motion_cfg_fn_arg, retarget_cfg_fn_arg)
