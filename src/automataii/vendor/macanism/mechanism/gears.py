...
class SpurGear:
    ...
    def __init__(
        ...
        if N and pd:
            ...
            if isinstance(self.N, float):
                assert self.N.is_integer(), (
                    "The calculated number of teeth is not registering as an integer. "
        ...
        if agma:
            assert pressure_angle == 20 or pressure_angle == 25, (
                "AGMA standards only apply to 20 or 25 degree pressure angles."
            )
            if self.pd >= 20:
                assert pressure_angle == 20, (
                    "For diametral pitches greater than or equal to 20, the pressure angles "
                    "must be 20 degrees."
                )
            ...
            self.a, self.b = a, b
            assert a > 0 and b > 0, (
                "Addendum and dedendum need to be defined if the agma argument is not present."
            )
            self.backlash = backlash
            assert backlash >= 0, "Use a positive value for backlash."
            backlash = backlash if not internal else -backlash
        ...
        if rb < self.r_base and not ignore_undercut:
            warnings.warn(
                "The dedendum circle radius is less than the base circle radius. Undercutting will occur. To "
        ...
        if rb >= self.r_base:
            theta_min = np.sqrt(
                (rb / self.r_base) ** 2 - 1
            )  # Relationship is a result of converting to polar form
        ...
        self.involute_reflection = -1 * np.real(self.involute_points) + 1j * np.imag(
            self.involute_points
        )
        addendum_start = np.angle(self.involute_points[-1])
        addendum_end = np.angle(self.involute_reflection[-1])
        addendum_circle = ra * np.exp(
            1j * np.linspace(addendum_start, addendum_end, size)
        )

        # Construct the tooth profile
        self.tooth_profile = np.concatenate(
            (
                self.involute_points,
                addendum_circle[1:-1],
    ...
    def plot(self, grid=True):
        ...
        dedendum_angle = np.angle(self.involute_reflection[0]) - np.angle(
            self.involute_points[0]
        )
        ...
        dedendum_draw = np.linspace(
            angle_start, np.angle(self.involute_points[0]), 1000
        )
        ...
        rb = (
            self.r - self.b if not self.internal else self.r - self.a
        ...
        ax.plot(
            np.real(self.tooth_profile),
            np.imag(self.tooth_profile),
            **SpurGear.gear_appearance["tooth"],
        )
        ax.plot(
            np.real(dedendum), np.imag(dedendum), **SpurGear.gear_appearance["tooth"]
        )
        ax.plot(
            -1 * np.real(dedendum),
            np.imag(dedendum),
        ...
        with open(file, "w", newline="") as f:
            if solidworks:
                ...
                for c_num in self.tooth_profile:
                    writer.writerow(
                        [f"{np.real(c_num):.8f}", f"{np.imag(c_num):.8f}", 0]
                    )
            else:
                writer = csv.writer(f, delimiter=",")
                writer.writerow(["x", "y"])
