...
class Data:
    ...
    def __init__(self, matrix, find_headers=False, headers=None):
        if not isinstance(matrix, Data):
            ...
            if self.headers:
                self.data_dict = {
                    self.headers[c]: self._construct_column(self.headers[c])
    ...
    def print(self, table=False, underline=False, columns=None):
        ...
        if underline or table:
            underlines = [
                "-" * len(string_matrix[0][c]) for c in range(len(string_matrix[0]))
            ]
            string_matrix.insert(1, underlines)
            m += 1

        column_widths = [
            max([len(string_matrix[r][c]) for r in range(m)]) for c in range(n)
...
def read_csv(csv_file, delimiter=",", find_headers=True, floats=True):
    ...
    with open(csv_file, "r") as f:
        if not floats:
            return Data(
                list(csv.reader(f, delimiter=delimiter)), find_headers=find_headers
            )
        else:
