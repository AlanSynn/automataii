...
class Cam:
    def __init__(self, motion=None, degrees=False, omega=0.0, rotation="ccw", h=0.0062):
        """
        ...
        if not degrees:
            ...
            assert np.abs(np.sum(self.intervals) - 2 * np.pi) < 1e-8, (
                "Given degree increments don't add up to full rotation."
            )
        ...
        assert all([m[0].lower() in ("dwell", "rise", "fall") for m in self.motion]), (
            'Only "rise", "fall", and "dwell" are acceptable descriptions.'
        )
        ...
        if not self.omega:
            self.naive = Naive(
                self.motion, self.shifts, self.intervals, self.conditions, self.thetas
            )
        ...
        assert self.rotation == "ccw" or self.rotation == "cw", (
            "Only 'ccw' or 'cw' must be specified for rotation."
        )
    ...
    def _get_conditions(self, t):
        ...
        for i in range(1, self.shifts.size):
            conditions.append(
                np.logical_and(t >= self.shifts[i - 1], t < self.shifts[i])
            )
        return conditions
    ...
    def profile(
        ...
        if kind == "all":
            ...
            if show_pitch:
                assert roller_radius, (
                    "Must include the roller radius to show the pitch."
                )
        ...
        if show_base:
            ...
            ax.plot(
                np.real(c_nums), np.imag(c_nums), **self.naive.cam_plot["base_circle"]
            )
    ...
    def get_base_circle(
        ...
        if follower == "roller":
            assert roller_radius is not None and max_pressure_angle is not None, (
                Exception(
            ...
            phi_max = lambda Rb: phi - np.max(
                np.abs(
                    np.arctan(
                        (y_ - eccentricity)
                        / (y + np.sqrt((Rb + Rf) ** 2 + eccentricity**2))
                    )
                )
            )
            base_radius = fsolve(phi_max, np.array([0.1]))[0]
            ...
            assert min_rho > roller_radius, (
                "Calculated base radius results in a minimum radius of curvature that is "
            ...
            if plot:
                ...
                phis = np.abs(
                    np.arctan(
                        (y_ - eccentricity)
                        / (y + np.sqrt((base_radius + Rf) ** 2 + eccentricity**2))
                    )
                )
                ax.plot(
                    self.thetas_d,
                ...
                ax.plot(
                    self.thetas_d,
                    max_pressure_angle * np.ones(self.thetas.size),
                    color="black",
                    ls="--",
                    label=rf"$\phi={max_pressure_angle}^\circ$",
                )
            ...
            if not conservative_flat:
                ...
                return {
                    "Rb": base_circle,
                    "Min Rho": desired_min_rho,
                    "Min Face Width": min_face_width,
                    "rhos": rhos,
                }
            ...
            raise Exception(
                'Only acceptable arguments for follower are "flat" and "roller".'
            )
    ...
    def get_animation(
        ...
        if roller_radius:
            ...
            items = [
                ax.fill([], [], **motion_type.cam_plot["fill"])[0],
                ax.fill([], [], **motion_type.cam_plot["follower_fill"])[0],
                ax.fill([], [], **motion_type.cam_plot["follower_fill"])[0],
            ]
        elif face_width:
            follower = FlatFollower(
                motion_type,
                base,
                self.thetas_r,
                inc,
        ...
        def animate_roller(index):
            items[0].set_xy(
                np.stack((follower.cam_x[index], follower.cam_y[index]), axis=1)
            )
            items[1].set_xy(
                np.stack((follower.roller_x[index], follower.roller_y[index]), axis=1)
            )
        ...
        return (
            FuncAnimation(
                fig,
                animate,
                frames=range(follower.motion_length),
                interval=20,
                blit=True,
                init_func=init,
            ),
...
class Motion:
    def __init__(self, motion, shifts, intervals, conditions, thetas, omega=None):
        ...
        self.S = np.piecewise(
            self.thetas, condlist=self.conditions, funclist=self._get_functions(self.f)
        )
    ...
    def _get_functions(self, func_maker, rate=False):
        ...
        if start[0].lower() == "dwell":
            ...
            h1 += h2
            assert start[0].lower() != "fall", (
                "Displacement must be positive for all rotation; therefore, it can't "
                "with a fall."
            )
        ...
        for i in range(1, self.shifts.size):
            ...
            if motion[0].lower() == "dwell" and not rate:
                ...
                functions.append(
                    func_maker(h1, h2, self.intervals[i], self.shifts[i - 1])
                )
                h1 += h2
        return functions
    ...
    def _get_rates(self):
        ...
        self.V = (
            np.piecewise(
                ...
                funclist=self._get_functions(self.f_, rate=True),
            )
            * self.omega
        )
        self.A = (
            np.piecewise(
                self.thetas,
                condlist=self.conditions,
                funclist=self._get_functions(self.f__, rate=True),
            )
            * self.omega**2
        )
        self.J = (
            np.piecewise(
                self.thetas,
                condlist=self.conditions,
                funclist=self._get_functions(self.f___, rate=True),
            )
            * self.omega**3
        )
...
class Naive(Motion):
    def __init__(self, motion, shifts, intervals, conditions, thetas, omega=None):
        Motion.__init__(
            self, motion, shifts, intervals, conditions, thetas, omega=omega
        )
...
class Harmonic(Motion):
    ...
    def f__(_, h2, B, s):
        return (
            lambda theta: np.pi**2 * h2 * np.cos(np.pi * (theta - s) / B) / (2 * B**2)
        )

    @staticmethod
    def f___(_, h2, B, s):
        return (
            lambda theta: -(np.pi**3)
            * h2
            * np.sin(np.pi * (theta - s) / B)
            / (2 * B**3)
        )
...
class Cycloidal(Motion):
    ...
    @staticmethod
    def f(h1, h2, B, s):
        return lambda theta: h1 + h2 * (
            (theta - s) / B - 1 / (2 * np.pi) * np.sin(2 * np.pi * (theta - s) / B)
        )
    ...
    def f___(_, h2, B, s):
        return (
            lambda theta: 4 * np.pi**2 * h2 * np.cos(2 * np.pi * (theta - s) / B) / B**3
        )
...
class RollerFollower:
    def __init__(
        ...
        self.cam_x, self.cam_y = (
            [],
            [],
        )  # A list of x and y coordinates of the cam profile for each rotation
        self.roller_centers = []  # A list of x coordinates of the roller center
        self.roller_x, self.roller_y = (
            [],
            [],
        )  # A list of x and y coordinates for the circle of the roller
    ...
    def plot(self, grid=True):
        ...
        if grid:
            ax.grid(zorder=1)
        ax.plot(
            np.rad2deg(self.motion.thetas[self.indexes]),
            self.S,
            label="Follower Displacement",
...
class FlatFollower:
    ...
    def __init__(
        self, motion, base, thetas, inc, face_width=0, length=0, width=0, eccentricity=0
    ):
        self.indexes = range(0, thetas.size, inc)
        self.motion_length = len(self.indexes)
        self.motion = motion
        ...
        for i in range(self.motion_length):
            ...
            self.cam_x.append(x)
            self.cam_y.append(y)
            possible = np.logical_and(
                y >= -(face_width / 2 - eccentricity),
                y <= face_width / 2 + eccentricity,
            )
