...
m = (
    N_gear / N_pinion
)  # gear ratio (by definition, gear is always bigger than the pinion)
...
def loops(x, inp):
    ...
    temp = np.concatenate(
        (temp, [a.pos.theta + m * c.pos.theta])
    )  # equation 3 appended
    return temp
...
mech = Mechanism(
    vectors=(a, b, c, d), origin=O, loops=loops, pos=thetas, guess=(pos_guess,)
)
mech.iterate()
