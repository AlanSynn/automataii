...
cam = Cam(
    motion=[("Dwell", 90), ("Rise", 1, 90), ("Dwell", 90), ("Fall", 1, 90)],
    degrees=True,
    omega=2 * np.pi,
)
...
flat_analysis = cam.get_base_circle(
    kind="cycloidal", follower="flat", desired_min_rho=0.25
)
...
ani, fig5, ax5, follower = cam.get_animation(
    kind="cycloidal",
    base=roller_analysis["Rb"],
    roller_radius=1 / 2,
    length=2,
...
ani_flat, fig7, ax7, follower = cam.get_animation(
    kind="cycloidal",
    base=flat_analysis["Rb"],
    face_width=2.75,
    length=2,
    width=3 / 8,
    inc=5,
