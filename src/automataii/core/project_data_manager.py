...
from pydantic import ValidationError, BaseModel, Field, validator, RootModel, field_validator

# Import runtime PartInfo and Pydantic models
from .models import PartInfo # Runtime PartInfo class
from .models_pydantic import ProjectFileModel as PydanticProjectFileModel, CharacterDataModel as PydanticCharacterDataModel, PartInfoModel as PydanticPartInfoModel, SkeletonJointModel as PydanticSkeletonJointModel # Pydantic models
...
class ProjectDataManager(QObject):
    ...
    project_data_loaded = pyqtSignal(bool, str, dict)  # success: bool, project_directory_path: str, parts_info: Dict[str, PartInfo]
    ...
    def __init__(self, parent: Optional[QObject] = None):
        ...
        self._validated_project_data: Optional[PydanticProjectFileModel] = None # Store the validated Pydantic model
    ...
    def raw_skeleton_data(self) -> Optional[List[Dict[str, Any]]]:
        ...
        if self._validated_project_data and self._validated_project_data.character:
            ...
            return [joint.model_dump() for joint in self._validated_project_data.character.skeleton_joints]
        return None
    ...
    def load_project_from_file(self, filepath: str) -> bool:
        ...
        try:
            ...
            if not fp.exists() or not fp.is_file():
                ...
                self.project_data_loaded.emit(False, "", {}) # Keep existing signal emission for failure
                return False
            ...
            if self._validated_project_data.character and self._validated_project_data.character.parts:
                for part_name, pydantic_part_model in self._validated_project_data.character.parts.items():
                    ...
                    if runtime_part_info.roi: # roi is [x, y, width, height]
                        all_part_rects.append(QRectF(runtime_part_info.roi[0], runtime_part_info.roi[1],
                                                     runtime_part_info.roi[2], runtime_part_info.roi[3]))
            ...
            num_skeleton_joints = len(self._validated_project_data.character.skeleton_joints) if self._validated_project_data.character else 0

            logging.info(f"ProjectDataManager: Successfully validated and parsed {num_loaded_parts} parts. Project dir: {self._project_dir}")
            if num_skeleton_joints > 0:
                logging.info(f"ProjectDataManager: Validated {num_skeleton_joints} skeleton joints.")
            ...
            self.error_occurred.emit(f"Invalid project file format: {ve.errors()}") # Send Pydantic errors
    ...
    def _try_load_supplemental_skeleton_data(self):
        ...
        """
        if not self._project_dir or not self._validated_project_data or not self._validated_project_data.character:
            logging.debug("Project dir or character data not available for supplemental skeleton load.")
            return

        # Check if skeleton data is already populated from the main project file
        if self._validated_project_data.character.skeleton_joints and len(self._validated_project_data.character.skeleton_joints) > 0:
        ...
        if not char_cfg_path.exists():
            logging.info(f"char_cfg.yaml not found at {char_cfg_path}, no supplemental skeleton to load.")
            return
        ...
        try:
            ...
            if not char_cfg_data or "skeleton" not in char_cfg_data or not isinstance(char_cfg_data["skeleton"], list):
            ...
            for joint_data_raw in supplemental_joints_raw:
                ...
                try:
                    ...
                    logging.warning(f"Validation error for a joint in char_cfg.yaml: {joint_data_raw}. Error: {ve_joint}")
                except Exception as e_joint:
                    logging.warning(f"Error processing a joint in char_cfg.yaml: {joint_data_raw}. Error: {e_joint}")


            if pydantic_skeleton_joints:
                self._validated_project_data.character.skeleton_joints = pydantic_skeleton_joints
                logging.info(f"Successfully loaded and validated {len(pydantic_skeleton_joints)} joints from {char_cfg_path}.")
            else:
            ...
            logging.error(f"Unexpected error loading supplemental skeleton from {char_cfg_path}: {e}", exc_info=True)
    ...
    def save_project_as_dialog(self):
        ...
        if self._project_dir and self._validated_project_data and self._validated_project_data.character:
            # Use character name for a more descriptive default, if available
            char_name = self._validated_project_data.character.name
            safe_char_name = "".join(c if c.isalnum() or c in (' ', '_', '-') else '' for c in char_name).strip()
            if safe_char_name:
...
if __name__ == '__main__':
    ...
    dummy_parts_info_content = {
        "character": {
            ...
            "parts": {
                "head": {"name": "head", "svg_path_file": "head.svg", "roi": [10,10,20,20], "z_value": 1},
                "torso": {"name": "torso", "svg_path_file": "torso.svg", "roi": [0,30,40,50], "z_value": 0}
            },
            "skeleton_joints": [
    ...
    def on_loaded_pydantic(success, proj_dir, parts_info):
        logging.info(f"PYDANTIC TEST: Caught project_data_loaded: Success={success}, Dir='{proj_dir}'")
        if success:
            logging.info(f"  Offset: {manager.effective_bounding_box_offset}")
            all_runtime_parts = parts_info
            logging.info(f"  Loaded runtime parts ({len(all_runtime_parts)}): {list(all_runtime_parts.keys())}")
            if all_runtime_parts.get('head'):
                logging.info(f"    Head ROI (runtime): {all_runtime_parts['head'].roi}")
                logging.info(f"    Head QPainterPath (runtime): {all_runtime_parts['head'].qpainter_path}")
            raw_skel_data = manager.raw_skeleton_data
            if raw_skel_data:
                logging.info(f"  Raw skeleton joints from Pydantic ({len(raw_skel_data)}): {raw_skel_data[0] if raw_skel_data else 'None'}")
    ...
    if os.path.exists(invalid_filepath): os.remove(invalid_filepath)
