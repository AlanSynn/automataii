...
from automataii.core import EventBus, get_global_event_bus
...
class ProjectManager(Injectable):
    ...
    def create_project(
        ...
        with self._lock:
            ...
            event = ProjectCreated(
                aggregate_id=str(id(project)), project_name=name, template_used=template
            )
            ...
            with self._lock:
                ...
                if self._current_project.file_path:
                    self._add_to_recent_projects(
                        self._current_project.file_path, self._current_project.name
                    )
    ...
    def close_project(self, save_if_modified: bool = True) -> bool:
        ...
        try:
            with self._lock:
                ...
                # Fire event
                event = ProjectClosed(aggregate_id=str(id(self)), project_name=project_name)
                self._event_bus.publish(event)
    ...
    def _start_auto_save(self) -> None:
        ...
        self._auto_save_timer = threading.Timer(self._auto_save_interval, auto_save_callback)
    ...
    def _add_to_recent_projects(self, file_path: Path, project_name: str) -> None:
        ...
        self._recent_projects = [p for p in self._recent_projects if p["path"] != str(file_path)]
    ...
    def _load_default_templates(self) -> None:
        ...
        self._templates = {
            "default": {
                ...
                "project_data": {"version": "1.0.0", "settings": {}},
            },
            "tutorial": {
                "name": "Tutorial Project",
                "description": "Sample project for learning",
                "project_data": {"version": "1.0.0", "tutorial_step": 0, "settings": {}},
            },
            "showcase": {
                "name": "Showcase Project",
                "description": "Demonstration of features",
                "project_data": {"version": "1.0.0", "demo_mode": True, "settings": {}},
            },
        }
