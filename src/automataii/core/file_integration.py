...
from automataii.core.di import Injectable
...
class FileIntegration(Injectable):
    ...
    def _register_windows_associations(self, app_path: Path) -> bool:
        ...
        try:
            ...
            with winreg.CreateKey(
                winreg.HKEY_CURRENT_USER, rf"Software\Classes\{self._file_extension}"
            ) as key:
    ...
    def _generate_default_thumbnail(self, project: AtiiProject, size: tuple) -> bytes:
        ...
        draw.text((width - type_width - 10, height - 25), type_text, fill="#666666", font=font)
    ...
    def get_file_info(self, project_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            return {"name": project_path.stem, "error": str(e)}
    ...
    def _find_or_create_app_bundle(self, app_path: Path) -> Path | None:
...
"""
    ...
    def _create_desktop_file(self, app_path: Path) -> Path | None:
