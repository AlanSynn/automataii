...
from automataii.core.models.project import (
...
class ProjectDataManager(QObject):
    ...
    def load_project_from_file(self, filepath: str) -> bool:
        ...
        self._clear_project_data_silent()  # Clear without emitting signal

        logging.info(f"ProjectDataManager: Cleared project data, now starting load process")

        try:
            fp = Path(filepath)
            logging.info(f"ProjectDataManager: Checking file path: {fp}, exists: {fp.exists()}, is_file: {fp.is_file() if fp.exists() else 'N/A'}")
            ...
            logging.info(f"ProjectDataManager: Opening and reading JSON file: {filepath}")
            with open(filepath, "r", encoding="utf-8") as f:
                data = json.load(f)
            logging.info(f"ProjectDataManager: JSON loaded successfully, data keys: {list(data.keys()) if isinstance(data, dict) else 'Not a dict'}")

            # Validate data with Pydantic models
            logging.info(f"ProjectDataManager: About to validate data with keys: {list(data.keys())}")
            validated_data = PydanticProjectFileModel.model_validate(data)
            self._validated_project_data = validated_data
            logging.info(f"ProjectDataManager: Data validation successful")
            ...
            logging.error(f"ProjectDataManager: Validation errors: {ve.errors()}")
        ...
        logging.error(f"ProjectDataManager: Loading failed, clearing project data and emitting failure")
        self._clear_project_data_silent()  # Ensure clean state on error
    ...
    def _clear_project_data_silent(self):
        """Clear project data without emitting signals (for internal use during loading)."""
        logging.info("ProjectDataManager: Clearing project data (silent).")
    ...
    def clear_project_data(self):
        """Clear project data and emit cleared signal."""
