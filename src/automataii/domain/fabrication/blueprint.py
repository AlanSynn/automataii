...
def generate_single_large_blueprint(
    layout_items, page_width_mm, page_height_mm, title="Manufacturing Blueprint", scale_info=""
):
    """
...
'''
    ...
    if part_items:
        ...
        svg_parts.append(
            '<text x="0" y="25" class="manufacturing-note">Cut on RED lines | Material: 3mm Plywood/Acrylic</text>'
        )
    ...
    if mechanism_items:
        ...
        for item in mechanism_items:
            ...
            # Add mechanism with its SVG content (sanitized)
            svg_parts.append(f'<g transform="translate({mech_x},{mech_y})">')
            # Implement comprehensive XSS protection using proper XML/SVG parsing
            try:
                from xml.etree import ElementTree as ET
                import xml.sax.saxutils as saxutils
                
                # First, escape HTML entities to prevent basic XSS
                sanitized_svg_content = saxutils.escape(str(item.svg_content), {"\"": "&quot;", "'": "&apos;"})
                
                # Parse as XML to validate structure and remove dangerous elements
                try:
                    # Wrap in root element for parsing
                    wrapped_content = f"<root>{sanitized_svg_content}</root>"
                    root = ET.fromstring(wrapped_content)
                    
                    # Remove dangerous elements and attributes
                    dangerous_elements = ['script', 'object', 'embed', 'iframe', 'link', 'meta', 'base']
                    dangerous_attrs = ['onload', 'onerror', 'onclick', 'onmouseover', 'onmouseout', 'onfocus', 'onblur']
                    ...
                    for elem in root.iter():
                        ...
                        for attr in elem.attrib:
                            if (attr.lower().startswith('on') or 
                                attr.lower() in dangerous_attrs or
                                'javascript:' in str(elem.attrib[attr]).lower() or
                                'data:' in str(elem.attrib[attr]).lower()):
                    ...
                    sanitized_svg_content = ''.join(ET.tostring(child, encoding='unicode') for child in root)
                    
                except ET.ParseError:
                    # If XML parsing fails, use regex-based fallback with enhanced patterns
                    sanitized_svg_content = re.sub(
                        r'<(script|object|embed|iframe|link|meta|base)[^>]*>.*?</\1>',
                        '', 
                    ...
                    sanitized_svg_content = re.sub(
                        r'(href|src|action|formaction|background|poster|cite|data|codebase)\s*=\s*["\']?(javascript:|data:)[^"\'>\s]*',
                        '',
                        sanitized_svg_content,
                        flags=re.IGNORECASE
                    )
                
                # Additional validation: ensure content only contains safe SVG elements
                safe_svg_pattern = re.compile(r'^[^<]*(<(\w+)[^>]*>[^<]*</\2>|<\w+[^>]*/>|<!--[^>]*-->)*[^<]*$')
                if not safe_svg_pattern.match(sanitized_svg_content):
                    logger.warning(f"SVG content failed safety validation, using fallback")
                    sanitized_svg_content = f'<text x="10" y="10" font-size="12" fill="red">Invalid SVG content removed</text>'
                
                svg_parts.append(sanitized_svg_content)
                
            except Exception as e:
                logger.error(f"Error sanitizing SVG content: {e}")
                # Safe fallback content
                svg_parts.append(f'<text x="10" y="10" font-size="12" fill="red">SVG content sanitization failed</text>')
            svg_parts.append("</g>")

            # Sanitize item name for logging to prevent log injection
            safe_item_name = html.escape(str(getattr(item, "name", "Unknown")), quote=True)
            safe_item_name = re.sub(r"[\\r\\n\\t]", "", safe_item_name)  # Remove newlines/tabs
            safe_svg_content = html.escape(str(item.svg_content[:100]), quote=True)
            logger.debug(
                f"Added mechanism at ({mech_x},{mech_y}): {safe_item_name} - {safe_svg_content}..."
            )
    ...
    for item in part_items:
        try:
            ...
            if hasattr(item, "part_info") and item.part_info:
                raw_part_name = getattr(item.part_info, "name", "Unknown Part")
                # Sanitize part name to prevent XSS
                part_name = html.escape(str(raw_part_name), quote=True)
                # Limit length to prevent DoS
                if len(part_name) > 100:
                    part_name = part_name[:100] + "..."
...
def _create_detailed_part_svg(
    polygon: QPolygonF, x_offset: float, y_offset: float, bounds: QRectF, part_name: str
) -> str:
...
def generate_blueprint_svg(part_items: list, padding: float = 20.0) -> str:
    ...
    for item in part_items:
        ...
        if not hasattr(item, "shape") or not callable(item.shape):
            part_name = (
                getattr(getattr(item, "part_info", None), "name", "Unknown")
                if hasattr(item, "part_info")
                else "Unknown"
            )
            logging.warning(f"Skipping item {part_name}: No shape method found.")
            continue

        part_path: QPainterPath = item.shape()  # Get the outline path in item's local coords
        ...
        for i in range(transformed_path.elementCount()):
            ...
            if element.isMoveTo():
                ...
                endpt = transformed_path.elementAt(i + 3)  # CurveToData includes endpoint
                ...
                if i + 2 < transformed_path.elementCount():
                    ...
                    logging.warning("CurveToData encountered - SVG conversion might be basic.")
                else:
        ...
        if svg_d:
            # Implement comprehensive SVG path sanitization
            try:
                clean_svg_d = svg_d.strip()
                
                # First validation: check for valid SVG path characters only
                valid_path_pattern = re.compile(r'^[MmLlHhVvCcSsQqTtAaZz0-9.,\s\-+eE]+$')
                if not valid_path_pattern.match(clean_svg_d):
                    logging.warning(f"Invalid SVG path data detected, contains non-path characters: {clean_svg_d[:50]}...")
                    continue
                
                # Remove any potential HTML/XML tags that might have been injected
                clean_svg_d = re.sub(r'<[^>]*>', '', clean_svg_d)
                
                # Validate path commands are properly formatted
                ...
                for pattern in suspicious_patterns:
                    if re.search(pattern, clean_svg_d, re.IGNORECASE):
                        logging.warning(f"Suspicious pattern detected in SVG path: {pattern}")
                        continue
                
                # Additional length validation to prevent DoS
                if len(clean_svg_d) > 10000:  # Reasonable limit for SVG paths
                    logging.warning(f"SVG path too long ({len(clean_svg_d)} chars), truncating")
                    clean_svg_d = clean_svg_d[:10000]
                
                # Final escape for XML safety
                clean_svg_d = html.escape(clean_svg_d, quote=True)
                ...
                logging.error(f"Error sanitizing SVG path data: {e}")
                # Skip this path rather than including potentially dangerous content
                continue
