...
from automataii.domain.fabrication.contour_extractor import ManufacturingContour
...
class ScaleNormalizer:
    ...
    def calculate_scale_factor(self, original_height_pixels: float) -> float:
        ...
        self.logger.info(
            f"Calculated scale factor: {scale_factor:.3f} mm/pixel (target: {self.target_height_mm}mm)"
        )
        return scale_factor
    ...
    def get_scaled_bounds(
        self, original_bounds: tuple[int, int, int, int], scale_factor: float
    ) -> ScaledBounds:
        """Convert pixel bounds to scaled real-world bounds"""
        x, y, w, h = original_bounds
        return ScaledBounds(
            x=x * scale_factor, y=y * scale_factor, width=w * scale_factor, height=h * scale_factor
        )
        ...
        for item in sorted_items:
            ...
            if current_x + item.bounds.width <= self.page_width_mm - self.padding_mm:
                ...
                new_bounds = ScaledBounds(
                    x=current_x, y=current_y, width=item.bounds.width, height=item.bounds.height
                )
...
class EnhancedMechanismProcessor:
    ...
    def process_mechanism(self, mech_id: str, mech_data: dict[str, Any]) -> LayoutItem | None:
        """
        ...
        try:
            ...
            if "real_world_params" in mech_data and "total_scale_factor" in mech_data:
                ...
                self.logger.info(
                    f"Using screen-calculated dimensions for {mech_id}: "
                ...
                self.logger.debug(
                    f"Using standard dimensions for {mech_id}: "
            ...
            self.logger.debug(
                f"Processed mechanism {mech_id}: {actual_width:.1f}x{actual_height:.1f}mm"
            )
            return layout_item
    ...
    def _generate_mechanism_svg(
        self, mech_id: str, mech_data: dict[str, Any], bounds: ScaledBounds
    ) -> str:
        ...
        try:
            ...
            ):  # Fixed: GUI creates '4_bar_linkage' not 'linkage'
            ...
            self.logger.warning(
                f"Generator enhancement failed for {mechanism_type}: {e}, using standard"
            )
    ...
    def _generate_parameter_annotations(
        ...
        try:
            ...
            if "scale_factor_used" in real_world_params:
                ...
                annotations.append(
                    f'<text x="{annotation_x}" y="{y_offset + 5}" '
...
class BlueprintLayoutOptimizer:
    ...
    def optimize_blueprint_layout(
        self, part_items: list[Any], mechanism_data: dict[str, Any]
        ...
        self.logger.info(
            f"Blueprint optimization complete: {len(optimized_items)} items, "
