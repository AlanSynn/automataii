...
from automataii.domain.segmentation.segmentation_manager import SegmentationManager
...
class FastSkeletonSegmenter:
    ...
    def segment(self, mask: np.ndarray, joints_list: list) -> np.ndarray:
        """Simple segment method for fallback compatibility"""
        # Simple fallback method - return the character mask for torso
        if mask is not None and np.sum(mask) > 0:
            return mask
        return np.zeros_like(mask) if mask is not None else np.zeros((100, 100), dtype=np.uint8)
...
class BodyPartsExtractor:
                ...
                if isinstance(joints, dict):
                    ...
                    for joint_id, joint_data in joints.items():
                        if isinstance(joint_data, dict) and "position" in joint_data:
                            ...
                            if len(pos) >= 2:
                                # Use the joint_id directly as the joint name
    ...
    def _segment_body_parts(self) -> dict[str, np.ndarray]:
        """Advanced body part segmentation using strategic ML approach"""
        if self.mask is None or self.texture_relative_joint_map is None:
            return {}

        try:
            # Prepare skeleton data in the format expected by SegmentationManager
            skeleton_data = self._prepare_skeleton_data_for_segmentation()

            # Initialize the strategic segmentation manager
            segmentation_manager = SegmentationManager()

            # Convert texture to RGB format for segmentation
            if self.texture is not None:
                if len(self.texture.shape) == 4:  # RGBA
                    image_rgb = cv2.cvtColor(self.texture, cv2.COLOR_BGRA2RGB)
                elif len(self.texture.shape) == 3:  # BGR
                    image_rgb = cv2.cvtColor(self.texture, cv2.COLOR_BGR2RGB)
                else:
                    # Grayscale - convert to RGB
                    image_rgb = cv2.cvtColor(self.texture, cv2.COLOR_GRAY2RGB)
            else:
                print("Warning: No texture available, using placeholder")
                image_rgb = np.zeros((self.mask.shape[0], self.mask.shape[1], 3), dtype=np.uint8)

            # Perform strategic segmentation
            print("Performing strategic ML-based segmentation...")
            segmentation_result = segmentation_manager.segment(
                image=image_rgb,
                mask=self.mask,
                skeleton_data=skeleton_data,
                require_validation=False,  # Allow partial results for now
            )

            if segmentation_result.masks:
                print(
                    f"Strategic segmentation successful: found {segmentation_result.part_count} parts "
                    f"with {segmentation_result.strategy_name} strategy "
                    f"(avg confidence: {segmentation_result.average_confidence:.2f})"
                )
                part_masks = segmentation_result.masks.copy()
            else:
                print("Strategic segmentation failed, falling back to skeleton-based approach")
                part_masks = self._fallback_skeleton_segmentation()

        except Exception as e:
            print(f"Error in strategic segmentation: {e}")
            print("Falling back to skeleton-based approach")
            part_masks = self._fallback_skeleton_segmentation()

        # Ensure all expected parts have masks (fill missing with empty masks)
        for part_name in BODY_PARTS:
            if part_name not in part_masks:
                part_masks[part_name] = np.zeros_like(self.mask)

        # Post-process masks to ensure they are valid
        processed_masks = {}
        for part_name, mask in part_masks.items():
            if mask is not None and np.sum(mask) > 0:
                # Ensure mask is the correct type and size
                ...
                # Constrain to original character mask
                mask = cv2.bitwise_and(mask, self.mask)
                processed_masks[part_name] = mask
            else:
                processed_masks[part_name] = np.zeros_like(self.mask)

        return processed_masks

    def _prepare_skeleton_data_for_segmentation(self) -> dict:
        """Prepare skeleton data in the format expected by SegmentationManager"""
        if not self.texture_relative_joint_map or not self.char_cfg:
            return {}

        # Convert joint map to the expected format
        joints = {}
        for joint_name, (x, y) in self.texture_relative_joint_map.items():
            joints[joint_name] = {
                "id": joint_name,
                "name": joint_name,
        ...
        hierarchy = {}
        if "hierarchy" in self.char_cfg:
            hierarchy = self.char_cfg["hierarchy"]
        elif "skeleton" in self.char_cfg and isinstance(self.char_cfg["skeleton"], list):
            # Build hierarchy from skeleton list format
            for joint_data in self.char_cfg["skeleton"]:
                if "name" in joint_data and "parent" in joint_data and joint_data["parent"]:
                    parent = joint_data["parent"]
                    if parent not in hierarchy:
                        hierarchy[parent] = []
                    hierarchy[parent].append(joint_data["name"])

        return {
            "joints": joints,
            "hierarchy": hierarchy,
            "joint_map": {name: name for name in joints.keys()},  # Direct mapping
        }

    def _fallback_skeleton_segmentation(self) -> dict[str, np.ndarray]:
        """Fallback to the original skeleton-based segmentation approach"""
        ...
        # Use original fast segmenter as fallback
