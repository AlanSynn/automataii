...
class ARAP:
    ...
    def __init__(
        ...
        while (
            np.abs(np.linalg.det(tA1xA1_dense)) < 1e-12 and perturbation_count < max_perturbations
        ):
        ...
        if perturbation_count >= max_perturbations:
            logging.warning("tA1xA1 matrix remains singular after maximum perturbations")
        ...
        perturbation_count = 0
        while (
            np.abs(np.linalg.det(tA2xA2_dense)) < 1e-12 and perturbation_count < max_perturbations
        ):
        ...
        if perturbation_count >= max_perturbations:
            logging.warning("tA2xA2 matrix remains singular after maximum perturbations")
    ...
    def solve(self, pins_xy_: npt.NDArray[np.float32]) -> npt.NDArray[np.float64]:
        ...
        for idx, e0 in enumerate(self.edge_vectors):
            ...
            magnitude = np.sqrt(c * c + s * s)

            # Avoid division by zero
            if magnitude < 1e-12:
                logging.warning(
                    f"Near-zero magnitude {magnitude} in edge vector {idx}, using identity rotation"
                )
                T2 = np.eye(2)  # Identity matrix (no rotation)
            else:
                scale = 1.0 / magnitude
                c *= scale
                s *= scale
    ...
    def _xy_to_barycentric_coords(
        ...
        for p_xy in points:
            ...
            if np.any(np.abs(det_v1_v2) < 1e-12):
                logging.warning("Degenerate triangle detected, skipping barycentric calculation")
                continue

            a = (det(p_xy, v2) - det(v0, v2)) / det_v1_v2
            b = -(det(p_xy, v1) - det(v0, v1)) / det_v1_v2
    ...
    def _get_barycentric_coords(
        ...
        if np.abs(denom) < 1e-12:
            logging.warning(
                f"Degenerate triangle with near-zero denominator {denom}, using fallback coordinates"
            )
            # Return the first vertex coordinates as a fallback
