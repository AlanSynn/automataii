...
from automataii.core.models_pydantic import ProjectFileModel
...
from automataii.gui.tabs.mechanism_design.tab import MechanismDesignTab
from automataii.gui.tabs.options_tab import OptionsTab
from automataii.gui.views.editor_view import EditorView
from automataii.kinematics.kinematics_system import KinematicsSystem
...
class AutomataDesigner(QMainWindow):
    ...
    def _init_tabs(self):
        ...
        self.options_tab = OptionsTab(
            initial_anim_duration=self.kinematics_system.ik_animator.duration
        )
    ...
    def _handle_project_data_cleared(self):
        ...
        self.skeleton_manager.clear_data() # This will propagate to KinematicsSystem
    ...
    def handle_parts_generated_from_tab(self, annotation_results: dict, final_bpe_char_dir_str: str):
        ...
        if not parts_info_json_path.exists():
            QMessageBox.critical(self, "Project Load Error", f"Could not locate parts_info.json.")
            return
        self.project_data_manager.load_project_from_file(str(parts_info_json_path))
