...
from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QScrollArea, QGroupBox, QTreeWidget, QTreeWidgetItem
)
from PySide6.QtCore import Qt, Signal, QPointF

from ..views.editor_view import EditorView
from ...core.models_pydantic import MechanismModel, PartModel, LinkageMechanismModel, CamMechanismModel, GearMechanismModel, CharacterModel
...
class DesignerTab(QWidget):
    ...
    mechanism_data_updated = Signal(MechanismModel) # When a mechanism is created or significantly changed
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.current_mechanisms: Dict[str, MechanismModel] = {} # Store mechanisms being designed
    ...
    def _init_ui(self):
        ...
        self.properties_placeholder_label = QLabel("Select a part or mechanism to see properties.")
    ...
    def _connect_signals(self):
        ...
        self.mechanisms_tree_widget.itemClicked.connect(self._on_mechanism_tree_item_clicked)
        ...
        if self.editor_view:
            ...
            self.editor_view.part_moved.connect(self._on_part_moved_in_editor) # (part_id, new_pos, new_rotation, new_scale)
            self.editor_view.mechanism_structure_changed.connect(self._on_mechanism_structure_changed_in_editor) # (mechanism_id, change_details)
    ...
    def _update_ui_for_active_data(self):
        ...
        if self.current_character_model and self.current_character_model.parts:
            for part_id, part_model in self.current_character_model.parts.items():
                item = QTreeWidgetItem(self.parts_tree_widget, [part_model.name or part_id])
                item.setData(0, Qt.ItemDataRole.UserRole, part_id) # Store part_id
                # item.setIcon(0, QIcon(":/icons/part_icon.png")) # Example icon
            self.parts_tree_group.setTitle(f"Parts for: {self.current_character_model.name}")
        ...
        if self.current_mechanisms:
            for mech_id, mech_model in self.current_mechanisms.items():
                item = QTreeWidgetItem(self.mechanisms_tree_widget, [mech_model.name or mech_id, str(mech_model.type.value)])
        ...
        if self.active_mechanism_id and self.active_mechanism_id in self.current_mechanisms:
            ...
            self.properties_placeholder_label.setText(f"Editing Mechanism: {active_mech_model.name}\nType: {active_mech_model.type.value}")

            if self.selected_part_id_in_editor and self.current_character_model and self.selected_part_id_in_editor in self.current_character_model.parts:
                active_part_model = self.current_character_model.parts[self.selected_part_id_in_editor]
                ...
                self.properties_placeholder_label.setText(f"Selected Part: {active_part_model.name} (from Character)\nMechanism: {active_mech_model.name}")
        ...
        elif self.selected_part_id_in_editor and self.current_character_model and self.selected_part_id_in_editor in self.current_character_model.parts:
            ...
            # self.part_properties_widget.setVisible(False)
            # self.mechanism_properties_widget.setVisible(False)
            self.properties_placeholder_label.setText("Select a part or mechanism to see properties.")

        # Update EditorView
        if self.editor_view:
            if self.active_mechanism_id and active_mech_model:
                self.editor_view.load_mechanism(active_mech_model, self.current_character_model.parts if self.current_character_model else None)
                ...
                self.editor_view.set_background_text("Load a Character (from Image Proc. tab) or create a Mechanism.")
    ...
    def _on_part_tree_item_clicked(self, item: QTreeWidgetItem, column: int):
        ...
        if part_id and self.current_character_model and part_id in self.current_character_model.parts:
            self.selected_part_id_in_editor = part_id # Tentative selection
            self.part_selection_changed.emit(part_id)
            self.status_message_updated.emit(f"Part '{self.current_character_model.parts[part_id].name}' selected from tree.")
            ...
            if self.active_mechanism_id and self.editor_view:
                self.editor_view.add_part_to_scene(self.current_character_model.parts[part_id])
                self.editor_view.select_part_item(part_id)
            elif self.editor_view: # No active mechanism, just show the part
                 self.editor_view.select_part_item(part_id) # Assumes parts are already loaded if character is there
        self._update_ui_for_active_data()
    ...
    def _on_mechanism_tree_item_clicked(self, item: QTreeWidgetItem, column: int):
        ...
        if mech_id and mech_id in self.current_mechanisms:
            ...
            self.selected_part_id_in_editor = None # Clear part selection when switching mechanisms
            self.part_selection_changed.emit(None)
            self.status_message_updated.emit(f"Mechanism '{self.current_mechanisms[mech_id].name}' selected.")
        self._update_ui_for_active_data()
    ...
    def _on_part_selected_in_editor(self, part_id: Optional[str]):
        ...
        self.part_selection_changed.emit(part_id)
        if part_id and self.current_character_model and part_id in self.current_character_model.parts:
            self.status_message_updated.emit(f"Part '{self.current_character_model.parts[part_id].name}' selected in editor.")
        elif part_id and self.active_mechanism_id and self.current_mechanisms[self.active_mechanism_id].parts and part_id in self.current_mechanisms[self.active_mechanism_id].parts:
             self.status_message_updated.emit(f"Part '{self.current_mechanisms[self.active_mechanism_id].parts[part_id].name}' (mechanism-specific) selected in editor.")
        else:
            self.status_message_updated.emit("Editor selection cleared.")
        self._update_ui_for_active_data()

    def _on_part_moved_in_editor(self, part_id: str, new_pos: QPointF, new_rotation: float, new_scale: float):
        ...
        if self.current_character_model and part_id in self.current_character_model.parts:
            target_part_model = self.current_character_model.parts[part_id]
        elif self.active_mechanism_id and self.current_mechanisms[self.active_mechanism_id].parts and part_id in self.current_mechanisms[self.active_mechanism_id].parts:
            target_part_model = self.current_mechanisms[self.active_mechanism_id].parts[part_id]
        ...
        if target_part_model:
            ...
            logging.debug(f"Part '{part_id}' moved/scaled. New state: x={new_pos.x():.1f}, y={new_pos.y():.1f}, rot={new_rotation:.1f}, scale={new_scale:.2f}")
            ...
            if self.active_mechanism_id and self.current_mechanisms[self.active_mechanism_id].is_part_of_mechanism(part_id):
                self._solve_and_update_active_mechanism()
        else:
            logging.warning(f"Moved part '{part_id}' not found in character or active mechanism models.")


    def _on_mechanism_structure_changed_in_editor(self, mechanism_id: str, change_details: Dict[str, Any]):
        ...
        if mechanism_id in self.current_mechanisms:
            ...
            logging.info(f"Mechanism '{mechanism_id}' structure changed via editor: {change_details}")
            # Example: if a joint position was changed, update the point in the model.
            if 'joint_updated' in change_details:
                joint_update_info = change_details['joint_updated'] # { 'joint_id': ..., 'new_pos': QPointF(...) }
                if hasattr(active_mech, 'points') and joint_update_info['joint_id'] in active_mech.points:
                    active_mech.points[joint_update_info['joint_id']] = joint_update_info['new_pos']
            ...
            logging.warning(f"Structure change received for unknown mechanism_id: {mechanism_id}")

    # --- Mechanism Creation Methods --- #

    def _create_new_mechanism_base(self, mech_type_enum, default_name_prefix: str) -> Optional[MechanismModel]:
        if not self.current_character_model:
            QMessageBox.warning(self, "No Character", "A character must be loaded to create mechanisms.")
            return None

        new_mech_id = f"{mech_type_enum.value.lower()}_{len(self.current_mechanisms) + 1}"
        ...
        if mech_type_enum == LinkageMechanismModel.model_fields['type'].default:
            model = LinkageMechanismModel(id=new_mech_id, name=new_mech_name, type=mech_type_enum)
        elif mech_type_enum == CamMechanismModel.model_fields['type'].default:
            model = CamMechanismModel(id=new_mech_id, name=new_mech_name, type=mech_type_enum)
        elif mech_type_enum == GearMechanismModel.model_fields['type'].default:
            model = GearMechanismModel(id=new_mech_id, name=new_mech_name, type=mech_type_enum)
        else:
            logging.error(f"Unsupported mechanism type for creation: {mech_type_enum}")
            QMessageBox.critical(self, "Error", f"Cannot create mechanism of type: {mech_type_enum}")
            return None
        ...
        logging.info(f"Created new {mech_type_enum.value} mechanism: {new_mech_id} - '{new_mech_name}'")
        self._update_ui_for_active_data()
        self.editor_view.set_mode('edit_mechanism') # Switch editor to mechanism editing mode
        return model

    def _create_new_linkage_mechanism(self):
        self._create_new_mechanism_base(LinkageMechanismModel.model_fields['type'].default, "Linkage")

    def _create_new_cam_mechanism(self):
        self._create_new_mechanism_base(CamMechanismModel.model_fields['type'].default, "Cam-Follower")

    def _create_new_gear_mechanism(self):
        self._create_new_mechanism_base(GearMechanismModel.model_fields['type'].default, "Gear Pair")
    ...
    def load_character_for_design(self, character_model: CharacterModel):
        ...
        self.status_message_updated.emit(f"Character '{character_model.name}' loaded for design.")
        logging.info(f"DesignerTab: Loaded character '{character_model.name}' (ID: {character_model.id}).")
        self._update_ui_for_active_data()
    ...
    def load_mechanisms_from_project_data(self, mechanisms_data: Dict[str, Dict[str, Any]]):
        ...
        for mech_id, data_dict in mechanisms_data.items():
            ...
            if isinstance(data_dict, MechanismModel):
                self.current_mechanisms[mech_id] = data_dict # if data_dict is already a model instance
            else: # If it's a dict, need to parse (example above)
                logging.warning(f"Mechanism data for {mech_id} is a dict, needs robust parsing. Skipping for now.")
                pass # Implement robust parsing

        self.active_mechanism_id = None
        if self.current_mechanisms:
            self.active_mechanism_id = list(self.current_mechanisms.keys())[0] # Activate first one

        logging.info(f"Loaded {len(self.current_mechanisms)} mechanisms into DesignerTab from project data.")
        self._update_ui_for_active_data()


    def _solve_and_update_active_mechanism(self):
        if not self.active_mechanism_id or self.active_mechanism_id not in self.current_mechanisms:
            return
        ...
        logging.debug(f"Placeholder: Solving kinematics for mechanism '{mech_model.name}'")
