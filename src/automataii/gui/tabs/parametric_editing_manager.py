...
class ParametricEditingManager:
    ...
    def __init__(self, parent_tab):
        ...
        # Physics snap mode: 'fast' | 'balanced' | 'high' (alert-only)
        # This can be surfaced via Options UI later; default balanced
    ...
    def _on_parametric_mechanism_update(self, mechanism_id: str, params: Dict[str, Any]):
        ...
        try:
            ...
            # Soft physics snapping based on mechanism type
            ...
            try:
                ...
                # Keep editing responsive even if a guard fails
                pass
    ...
    def _enforce_grashof_and_snap(self, layer_data: Dict[str, Any]) -> bool:
        """Enforce Grashof condition with minimal snapping on non-ground links.

        Keep ground link (between anchors) fixed. Adjust smaller middle link (prefer L3=coupler)
        by the minimal delta to satisfy s + l <= m1 + m2. Operates in mechanism units.
        """
        params = layer_data.get("params", {})
        # Get mech transforms
        to_mech = self.parent_tab._get_inverse_scene_transform_function(layer_data)
        ...
        p1 = to_mech(a1); p2 = to_mech(a2)
        p1 = np.array([float(p1[0]), float(p1[1])]); p2 = np.array([float(p2[0]), float(p2[1])])
        L1 = float(np.linalg.norm(p2 - p1))

        # Scene->mech length mapping using transform scale
        transform_params = layer_data.get("transform_params", {})
        scale = float(transform_params.get("scale", 1.0))
        user_scale = 100.0
        try:
            user_path_np = utils_qpainterpath_to_numpy_array(layer_data.get("generated_path"))
        ...
        def scene_to_mech_len(val: float) -> float:
            return float(val) * (scale / user_scale)
        def mech_to_scene_len(val: float) -> float:
            return float(val) * (user_scale / scale) if scale != 0 else float(val)
        ...
        # Prefer adjusting coupler (L3) if it's one of the middles
        target = None
        if "L3" in (m1_name, m2_name):
            target = "L3"
        else:
            target = m1_name  # smaller middle link
        ...
        # Skip snapping if policy says alert-only
        if self.physics_snap_mode == 'high':
            logging.warning("[PHYSICS-SNAP] Grashof violated; alert-only mode; target=%s needs +%.3f (mech)", target, delta)
            return False

        logging.warning("[PHYSICS-SNAP] Grashof violated; snapped %s by +%.3f (mech units)", target, delta)
        return True

    def _enforce_gear_meshing_and_snap(self, layer_data: Dict[str, Any]) -> bool:
        """Ensure gear centers are separated by r1+r2(+clearance), adjust gear2 position softly.

        Works in scene space using params gear1_x/gear1_y/gear2_x/gear2_y and gear1_radius/gear2_radius.
        Falls back to r1/r2 if scene radii are missing. Leaves radii unchanged; moves gear2 only.
        """
        params = layer_data.get("params", {})
        try:
            g1x = float(params.get("gear1_x")); g1y = float(params.get("gear1_y"))
            g2x = float(params.get("gear2_x")); g2y = float(params.get("gear2_y"))
        except Exception:
            return False

        # Radii – prefer scene radii if present
        ...
        import numpy as _np
        g1 = _np.array([g1x, g1y], dtype=float)
        g2 = _np.array([g2x, g2y], dtype=float)
        d = float(_np.linalg.norm(g2 - g1))
        if not _np.isfinite(d) or d < 1e-9:
            return False

        # Policy-based clearance
        ...
        # Alert-only policy
        if self.physics_snap_mode == 'high':
            logging.warning("[PHYSICS-SNAP] Gear mesh off by %.3f; alert-only (no snap)", d - desired)
            return False

        # Move gear2 along current line to desired distance
        dir_vec = (g2 - g1) / d
        new_g2 = g1 + dir_vec * desired
        params["gear2_x"] = float(new_g2[0])
        params["gear2_y"] = float(new_g2[1])
        logging.warning("[PHYSICS-SNAP] Adjusted gear2 to maintain mesh (Δ=%.3f)", desired - d)
        return True

    def _enforce_cam_follower_snap(self, layer_data: Dict[str, Any]) -> bool:
        """Keep cam follower within simple physical bounds using soft snap.

        - Clamp rod length to a minimum.
        - Ensure base_radius non-negative.
        This operates only on params; visuals and sim use these downstream.
        """
        params = layer_data.get("params", {})
        changed = False

        # Alert-only mode: early exit with diagnostics
        if self.physics_snap_mode == 'high':
            # Still check and log if off
            rod_len = float(params.get("follower_rod_length", 40.0))
            if rod_len < 20.0:
                logging.warning("[PHYSICS-SNAP] CAM rod length too short (%.2f) — alert-only", rod_len)
            base_r = float(params.get("base_radius", 25.0))
            if base_r < 0.0:
                logging.warning("[PHYSICS-SNAP] CAM base radius negative (%.2f) — alert-only", base_r)
            return False

        # Clamp follower rod length
        try:
            rod_len = float(params.get("follower_rod_length", 40.0))
            min_len = 20.0 if self.physics_snap_mode == 'fast' else 15.0
            if rod_len < min_len:
                params["follower_rod_length"] = float(min_len)
                logging.warning("[PHYSICS-SNAP] CAM: clamped follower_rod_length to %.1f", min_len)
                changed = True
        except Exception:
            pass

        # Clamp base radius to non-negative reasonable minimum
        try:
            base_r = float(params.get("base_radius", 25.0))
            if base_r < 1.0:
                params["base_radius"] = 1.0
                logging.warning("[PHYSICS-SNAP] CAM: clamped base_radius to 1.0")
                changed = True
        except Exception:
            pass
    ...
    # Optional: simple setter for external UI wiring later
    def set_physics_snap_mode(self, mode: str) -> None:
        mode = (mode or '').strip().lower()
        if mode in ('fast', 'balanced', 'high'):
            self.physics_snap_mode = mode
        else:
            logging.warning("[PHYSICS-SNAP] Unknown mode '%s'; keeping '%s'", mode, self.physics_snap_mode)
    ...
    def _regenerate_4bar_simulation(self, layer_data: Dict[str, Any], params: Dict[str, Any]):
        ...
        # Build mechanism-space pivots and lengths
        to_mech = self.parent_tab._get_inverse_scene_transform_function(layer_data)
        transform_params = layer_data.get("transform_params", {})
        scale = float(transform_params.get("scale", 1.0))
        user_scale = 100.0
        ...
        # Convert scene lengths to mechanism lengths if needed
