# Four-Bar Mechanism Implementation
# Lines: ~300
# Public API: FourBarMechanism
# Deps In: registry
# Deps Out: numpy, math, interfaces
# Coupling: Low (implements interface)
# Cohesion: Feature (4-bar linkage simulation)
# Owner: Alan Synn
...
class FourBarMechanism(MechanismInterface):
    ...
    def simulate(self, num_frames: int = 100) -> SimulationData:
        ...
        for i, theta in enumerate(time_steps):
    ...
    def _solve_rocker_position(self, p3: np.ndarray) -> Optional[np.ndarray]:
        """
        Solve for rocker position using circle-circle intersection.
