# Four-Bar Blueprint Generator
# Lines: ~400
# Public API: FourBarBlueprintGenerator
# Deps In: BlueprintGenerator
# Deps Out: math, numpy
# Coupling: Low
# Cohesion: Feature (four-bar blueprint generation)
# Owner: Alan Synn
...
class FourBarBlueprintGenerator(BlueprintGenerator):
    ...
    def _draw_revolute_joint(self, x: float, y: float, 
        ...
        <g id="joint-{label}">
            ...
            {'<line x1="{}" y1="{}" x2="{}" y2="{}"/>'.format(x-2, y, x+2, y) if is_fixed else ''}
            {'<line x1="{}" y1="{}" x2="{}" y2="{}"/>'.format(x, y-2, x, y+2) if is_fixed else ''}
            {'</g>' if is_fixed else ''}
