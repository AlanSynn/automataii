...
from PyQt5.QtWidgets import (
    ...
    QSplitter, QWidget, QMessageBox, QToolBar, QAction, QFileDialog
)
from PyQt5.QtCore import Qt, QPointF, pyqtSignal, QRectF
from PyQt5.QtGui import QPen, QBrush, QColor, QPainter, QIcon
...
class JointItem(QGraphicsEllipseItem):
    """Interactive joint item for the skeleton editor."""
    
    def __init__(self, joint_id: str, x: float, y: float, radius: float = 5.0):
        ...
        self.connections: List[BoneItem] = []
        
    def add_connection(self, bone: 'BoneItem'):
        """Add a bone connection to this joint."""
        self.connections.append(bone)
        
    def itemChange(self, change, value):
        """Handle item changes, updating connected bones."""
        if change == QGraphicsEllipseItem.ItemPositionHasChanged:
            # Update all connected bones
            for bone in self.connections:
...
class BoneItem(QGraphicsLineItem):
    ...
    def __init__(self, start_joint: JointItem, end_joint: JointItem):
        super().__init__()
        self.start_joint = start_joint
        self.end_joint = end_joint
        self.setPen(QPen(QColor(150, 150, 150), 3))
        self.setZValue(-1)  # Draw behind joints
        self.update_position()
        
        # Register with joints
        start_joint.add_connection(self)
        end_joint.add_connection(self)
        
    def update_position(self):
        """Update line position based on joint positions."""
        start_pos = self.start_joint.scenePos()
        end_pos = self.end_joint.scenePos()
