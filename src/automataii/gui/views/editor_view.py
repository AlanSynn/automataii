...
class EditorView(QGraphicsView):
    ...
    def __init__(self, scene, parent_window=None):
        ...
        logging.info(f"EditorView initialized with DPI: {self.dpi}, default unit: {self.display_unit}")
    ...
    def zoom(self, step: int):
        ...
        if new_zoom_level != self._zoom_level:
            ...
            if abs(target_scale - current_scale) < 0.00001 and ((step > 0 and self._zoom_level == self._max_zoom_level) or (step < 0 and self._zoom_level == self._min_zoom_level)):
                 self._zoom_level = new_zoom_level # Ensure level is updated if it was clamped
            ...
            if current_scale <= 0: # Safeguard for invalid current scale
                ...
                new_zoom_level = self._zoom_level + effective_step # Apply step from level 0
                new_zoom_level = max(self._min_zoom_level, min(new_zoom_level, self._max_zoom_level)) # Re-clamp
                target_scale = self._zoom_factor_base ** new_zoom_level
                target_scale = max(0.1, min(target_scale, 10.0))

            factor_to_apply = target_scale / current_scale if current_scale != 0 else target_scale # Avoid division by zero if current_scale somehow became 0
            if abs(factor_to_apply - 1.0) > 0.000001: # Only scale if factor is meaningfully different from 1
    ...
    def zoom_to_fit(self):
        ...
        self._show_status_message(f"Zoom to fit ({current_scale:.1f}x, level {self._zoom_level})")
