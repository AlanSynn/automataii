...
class Linkage(BaseMechanism):
    ...
    def generate(self, bar_type: str, **kwargs) -> dict[str, Any] | None:
        ...
        Args:
            bar_type: Type of linkage to generate (e.g., "3bar", "4bar").
            **kwargs: Arguments specific to the linkage type.
                      For "3bar": base_pos, scale, link_lengths, input_angle_deg
                      For "4bar": base_pos, scale, link_lengths, input_angle_deg

        Returns:
            A dictionary containing linkage data, or None if generation fails or type is unknown.
        """
        if bar_type == "3bar":
            return self._generate_3bar_data(**kwargs)
        elif bar_type == "4bar":
            return self._generate_4bar_data(**kwargs)
        else:
...
if __name__ == "__main__":
    ...
    print(f"Linkage Generator Description: {linkage_generator.get_description()}")

    # Mock data for testing
    # path = QPainterPath() # Not used in these examples
    # path.moveTo(10, 10)
    # path.lineTo(50, 50)
    # path.lineTo(10, 90)

    # pivot_a = QPointF(0, 0) # Not used directly in these examples
    # pivot_d = QPointF(100, 0) # Not used directly in these examples
    ...
    if linkage_4bar_fail is None:
        ...
        # print(linkage_4bar_fail) # Printing the dict might be too verbose if it's not None
...
class LinkageGenerator:
    ...
    def generate_svg(self, linkage_data: dict[str, Any]) -> str:
        ...
        try:
            ...
            center_x, _center_y = (min_x + max_x) / 2, (min_y + max_y) / 2
