    ...
    def _delegate_to_generator(
        ...
        try:
            if mechanism_type == '4_bar_linkage' and isinstance(mech_data.get('key_points'), dict):
                gen_bounds = LinkageBounds(x=bounds.x, y=bounds.y, width=bounds.width, height=bounds.height)
                return self._linkage_generator.generate_4bar_svg(
                    mech_data, gen_bounds,
                    fallback_generator=lambda mid, mt, b: self._generate_standard_mechanism_svg(mid, mt, bounds)
                )
                ...
                gen_bounds = GearBounds(x=bounds.x, y=bounds.y, width=bounds.width, height=bounds.height)
                ...
                gen_bounds = CamBounds(x=bounds.x, y=bounds.y, width=bounds.width, height=bounds.height)
