...
def solve_ik_ccd(chain, target_pos, iterations=10, tolerance=1.0, bend_directions=None):
    ...
    for i in range(len(chain)):
        ...
        if i > 0:
            ...
            logging.debug(
                f"IK: Bone length between {chain[i - 1].part_info.name} and {item.part_info.name}: {length:.2f}"
            )
    ...
    if target_distance > total_length:
        logging.debug(
            f"IK: Target unreachable. Distance: {target_distance:.2f}, Total length: {total_length:.2f}"
        )
    ...
    if len(chain) == 1:
        logging.warning(
            f"IK solver: Single item chain for {end_effector_item.part_info.name}. CCD requires at least 2 links."
        )
        return
    ...
    for iteration in range(iterations):
        if not end_effector_item.scene():
            logging.warning(
                f"End effector item {end_effector_item.part_info.name} not in scene at start of IK iteration."
            )
            return
        ...
        logging.debug(
            f"IK Iteration {iteration + 1}/{iterations}: Current EE Pos={current_ee_scene_pos}, Target={target_pos}, Error={error:.2f}"
        )

        if error < tolerance:
            logging.debug(
                f"IK solved in {iteration + 1} iterations, error: {error:.2f}"
            )
            return
        ...
        if len(chain) == 2:
            ...
            if chain[0].is_fixed or chain[0].is_joint_locked:
                ...
                logging.debug(
                    f"    Anchor Offset (local pivot on item): {item_to_rotate.anchor_offset}"
                )
                logging.debug(
                    f"    EndEffector Offset (local target point on item): {end_effector_local_pos}"
                )

                # Pivot is chain[1]'s anchor (its connection to chain[0])
                pivot_scene_pos_special = item_to_rotate.mapToScene(
                    item_to_rotate.anchor_offset
                )
                current_ee_scene_pos_special = item_to_rotate.mapToScene(
                    end_effector_local_pos
                )  # end_effector_local_pos is for chain[-1]

                logging.debug(
                    f"    Pivot Scene Pos (item's anchor in scene): {pivot_scene_pos_special}"
                )
                logging.debug(
                    f"    Current EE Scene Pos (item's EE point in scene): {current_ee_scene_pos_special}"
                )
                ...
                if (
                    QLineF(
                        pivot_scene_pos_special, current_ee_scene_pos_special
                    ).length()
                    < 1.0
                ):
                    logging.warning(
                        f"  IK 2-Link Special for '{item_to_rotate.part_info.name}': Pivot and End-Effector points are coincident ({pivot_scene_pos_special}). Skipping rotation this iteration."
                    )
                ...
                vec_pivot_to_ee_special = QLineF(
                    pivot_scene_pos_special, current_ee_scene_pos_special
                )
                vec_pivot_to_target_special = QLineF(
                    pivot_scene_pos_special, target_pos
                )

                line_pivot_to_ee_special = QLineF(
                    pivot_scene_pos_special, current_ee_scene_pos_special
                )
                line_pivot_to_target_special = QLineF(
                    pivot_scene_pos_special, target_pos
                )

                if (
                    line_pivot_to_ee_special.length() > 1e-6
                    and line_pivot_to_target_special.length() > 1e-6
                ):
                    angle_to_ee_deg_special = line_pivot_to_ee_special.angle()
                    angle_to_target_deg_special = line_pivot_to_target_special.angle()
                    angle_diff_deg_special = (
                        angle_to_target_deg_special - angle_to_ee_deg_special
                    )
                    while angle_diff_deg_special > 180:
                        angle_diff_deg_special -= 360
                    while angle_diff_deg_special < -180:
                        angle_diff_deg_special += 360
                    ...
                    logging.debug(
                        f"  IK 2-Link Special: Rotating '{item_to_rotate.part_info.name}': Old={old_rot_special:.2f}, Diff={angle_diff_deg_special:.2f}, New={new_rot_special:.2f}"
                    )
                    item_to_rotate.setRotation(new_rot_special)
                # After rotating the end-effector, recalculate error and check for early exit
                current_ee_scene_pos = end_effector_item.mapToScene(
                    end_effector_local_pos
                )
                error = QLineF(current_ee_scene_pos, target_pos).length()
                if error < tolerance:
                    logging.debug(
                        f"IK (2-link special) solved in {iteration + 1} iterations, error: {error:.2f}"
                    )
                    return
            ...
            if (chain[0].is_fixed or chain[0].is_joint_locked) and len(chain) == 2:  # Explicit 2-link fixed/locked base
                ...
                current_ee_on_item_scene_pos = item_to_rotate.mapToScene(
                    end_effector_local_pos
                )  # end_effector_local_pos is for chain[1]

                # Detailed logging as per previous steps
                logging.debug(
                    f"    2-Link Pivot (anchor of {item_to_rotate.part_info.name}): {pivot_scene_pos}"
                )
                logging.debug(
                    f"    2-Link EE point on {item_to_rotate.part_info.name} (local: {end_effector_local_pos}): {current_ee_on_item_scene_pos}"
                )
                ...
                if (
                    vec_pivot_to_current_ee.lengthSquared() < 1e-6
                ):  # Use a small tolerance for coincident points
                    logging.warning(
                    ...
                    if error_pivot_to_target < tolerance:
                        logging.debug(
                            f"IK 2-Link Special: Pivot for {item_to_rotate.part_info.name} is already at target (error: {error_pivot_to_target:.2f}). Considering solved."
                        )
                        return  # Solved
                    # Otherwise, this link cannot contribute. Since it's the only mobile link, we can't solve further.
                    logging.debug(
                        f"IK 2-Link Special: Pivot for {item_to_rotate.part_info.name} not at target (error: {error_pivot_to_target:.2f}), and EE point is coincident. Cannot solve further for this chain."
                    )
                    return  # Cannot improve

                # Angle calculation
                angle_rad = math.atan2(
                    vec_pivot_to_target.y(), vec_pivot_to_target.x()
                ...
                max_angle_deg = 30.0
                clamped_delta_angle_deg = max(
                    -max_angle_deg, min(max_angle_deg, delta_angle_deg)
                )

                if (
                    abs(clamped_delta_angle_deg) > 1e-3
                    ...
                    target_world_rotation = (
                        current_world_rotation + clamped_delta_angle_deg
                    )

                    # 로컬 회전 업데이트
                    current_local_rotation = item_to_rotate.rotation()
                    new_local_rotation = (
                        current_local_rotation + clamped_delta_angle_deg
                    )

                    logging.debug(
                        f"    2-Link Special '{item_to_rotate.part_info.name}': InitialWorld=0.0, CurrentWorld={current_world_rotation:.2f}, TargetWorld={target_world_rotation:.2f}, Delta={clamped_delta_angle_deg:.2f}, NewLocal={new_local_rotation:.2f}"
                    )
                    ...
                    logging.debug(
                        f"    2-Link Special '{item_to_rotate.part_info.name}': Delta angle {clamped_delta_angle_deg:.2f} too small, no rotation applied."
                    )
                ...
                current_ee_scene_pos = end_effector_item.mapToScene(
                    end_effector_local_pos
                )  # Recalculate global EE position
                error = QLineF(current_ee_scene_pos, target_pos).length()
                logging.debug(
                    f"IK Iteration {iteration + 1}/{iterations} (after 2-link adj): New EE Pos={current_ee_scene_pos}, Target={target_pos}, Error={error:.2f}"
                )
                if error < tolerance:
                    logging.debug(
                        f"IK solved in {iteration + 1} iterations (2-link path), error: {error:.2f}"
                    )
                    return
        ...
        for j in range(start_index_j, -1, -1):
            ...
            logging.debug(
                f"  IK Inner Loop: Processing '{current_item_to_rotate.part_info.name}' (index {j})"
            )
            ...
            pivot_scene_pos = current_item_to_rotate.mapToScene(
                current_item_to_rotate.anchor_offset
            )
            logging.debug(
                f"    Pivot Scene Pos: {pivot_scene_pos}, (Anchor Offset: {current_item_to_rotate.anchor_offset})"
            )
            ...
            if not end_effector_item.scene():
                logging.warning(
                    f"End effector item {end_effector_item.part_info.name} not in scene during IK inner loop."
                )
                break
            current_ee_scene_pos = end_effector_item.mapToScene(end_effector_local_pos)
            ...
            logging.debug(
                f"    Vec Pivot->EE: {vec_pivot_to_ee}, Vec Pivot->Target: {vec_pivot_to_target}"
            )
            ...
            logging.debug(
                f"    Angle EE_Line: {angle_to_ee_deg:.2f}, Angle Target_Line: {angle_to_target_deg:.2f}, Initial Diff: {angle_to_target_deg - angle_to_ee_deg:.2f}"
            )
            ...
            logging.debug(
                f"    Applying Rotation to '{current_item_to_rotate.part_info.name}': InitialWorld=0.0, CurrentWorld={current_world_rotation:.2f}, Delta={angle_diff_deg:.2f}, OldLocal={old_rotation:.2f}, NewLocal={new_rotation:.2f}"
            )
    ...
    if end_effector_item.scene():
        ...
        logging.debug(
            f"IK finished after {iterations} iterations, final error: {final_error:.2f}"
        )
    else:
...
def _enforce_bone_length_constraints(chain, bone_lengths, rotated_joint_index):
    ...
    for i in range(rotated_joint_index + 1, len(chain)):
        ...
        logging.debug(
            f"    Enforced bone length {target_length:.2f} between {parent_item.part_info.name} and {child_item.part_info.name}"
        )
...
def _solve_ik_fabrik(chain, bone_lengths, target_pos, iterations=10, tolerance=1.0, bend_directions=None):
    ...
    for i in range(len(chain)):
        ...
        if hasattr(item, "_initial_world_rotation"):
            ...
            if i < len(chain) - 1:
                ...
                angle_rad = math.atan2(
                    next_anchor.y() - current_anchor.y(),
    ...
    for iteration in range(iterations):
        ...
        if error < tolerance:
            logging.debug(
                f"FABRIK converged in {iteration + 1} iterations, error: {error:.2f}"
            )
            break
    ...
    for i in range(len(chain)):
        ...
        if i < len(chain) - 1:
            ...
            current_angle_rad = math.atan2(
                next_pos.y() - new_pos.y(), next_pos.x() - new_pos.x()
            )
            ...
            logging.debug(
                f"FABRIK: {item.part_info.name} - Initial angle: {initial_angles[i]:.1f}°, "
